...
Keep the repository-root `README.md` as the concise project catalog. Install
project-owned executables into `~/.local/bin` with `scripts/install.sh`, or set
`INSTALL_BIN_DIR` to choose another installation directory. Shell environments
and LaunchAgents should use installed command paths; shared tests should use
project-owned entry points. Preserve any installed application entry paths
under `apps/`. Shared tests, assets, package manifests, and repository-wide
...
- Re-run `scripts/install.sh` to install new project-owned executables into the
...
- If a CLI is renamed or removed, update or remove its root `README.md` entry,
  project README, and references to its installed command in the same change.
