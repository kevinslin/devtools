...
class ArborCliTest(unittest.TestCase):
    ...
    def test_clean_removes_merged_worktree_and_branch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            merge = self.run_git(repo, ["merge", "--no-ff", "-m", "merge feature", "feature/merged"])
            self.assertEqual(merge.returncode, 0, msg=merge.stderr)

            add_worktree = self.run_git(repo, ["worktree", "add", str(worktree_dir), "feature/merged"])
    ...
    def test_clean_keeps_unmerged_worktree_and_branch(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            back_to_main = self.run_git(repo, ["checkout", "main"])
            self.assertEqual(back_to_main.returncode, 0, msg=back_to_main.stderr)
            add_worktree = self.run_git(repo, ["worktree", "add", str(worktree_dir), "feature/open"])
            self.assertEqual(add_worktree.returncode, 0, msg=add_worktree.stderr)
    ...
    def test_clean_dry_run_previews_without_changes(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            back_to_main = self.run_git(repo, ["checkout", "main"])
            self.assertEqual(back_to_main.returncode, 0, msg=back_to_main.stderr)
            merge = self.run_git(repo, ["merge", "--no-ff", "-m", "merge preview", "feature/preview"])
            self.assertEqual(merge.returncode, 0, msg=merge.stderr)
            add_worktree = self.run_git(repo, ["worktree", "add", str(worktree_dir), "feature/preview"])
            self.assertEqual(add_worktree.returncode, 0, msg=add_worktree.stderr)
    ...
    def test_clean_force_removes_dirty_merged_worktree(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            back_to_main = self.run_git(repo, ["checkout", "main"])
            self.assertEqual(back_to_main.returncode, 0, msg=back_to_main.stderr)
            merge = self.run_git(repo, ["merge", "--no-ff", "-m", "merge dirty feature", "feature/dirty"])
            self.assertEqual(merge.returncode, 0, msg=merge.stderr)
            add_worktree = self.run_git(repo, ["worktree", "add", str(worktree_dir), "feature/dirty"])
            self.assertEqual(add_worktree.returncode, 0, msg=add_worktree.stderr)

            (worktree_dir / "dirty.txt").write_text("modified in worktree\n", encoding="utf-8")
