...
## Problem and Context
The repository currently contains a small utility script (`tools/jsonlint/bin/jsonlint`) and no existing audio or OpenAI integration flow. We need a focused tool that:
...
## Architecture
### A. CLI Layer
- Single executable script in `tools/beethoven/bin/beethoven.py` (or equivalent wrapper).
...
### C. Session Store
- Local storage under `tools/beethoven/.beethoven/sessions/<session_id>/`.
- Files:
...
2. Existing script at `tools/jsonlint/bin/jsonlint` suggests lightweight CLI utility style is appropriate.
