...
- `tools/agent-sync/bin/agent-sync`: `main` dispatches between `init` and sync mode, translates exceptions into exit codes, and prints the final JSON summary.
- `tools/agent-sync/bin/agent-sync`: `_init_config` materializes a default Codex-oriented JSON config in the current directory.
- `tools/agent-sync/bin/agent-sync`: `_sync` performs repo preparation, snapshot collection, sync planning, optional application, and summary generation.
...
- A user runs `python tools/agent-sync/bin/agent-sync ...` from a shell.

Entrypoints:
- `tools/agent-sync/bin/agent-sync:main`
- `tools/agent-sync/bin/agent-sync:_parse_init_args`
- `tools/agent-sync/bin/agent-sync:_parse_sync_args`
...
```ts
// Source: tools/agent-sync/bin/agent-sync
...
Entrypoints:
- `tools/agent-sync/bin/agent-sync:_init_config`
- `tools/agent-sync/bin/agent-sync:_default_init_config`
- `tools/agent-sync/bin/agent-sync:_sanitize_profile`
...
Entrypoints:
- `tools/agent-sync/bin/agent-sync:_load_config`
- `tools/agent-sync/bin/agent-sync:_prepare_repo`
- `tools/agent-sync/bin/agent-sync:_prepare_repo_for_dry_run`
- `tools/agent-sync/bin/agent-sync:_build_snapshot`
- `tools/agent-sync/bin/agent-sync:_load_state`
...
Entrypoints:
- `tools/agent-sync/bin/agent-sync:_sync`
- `tools/agent-sync/bin/agent-sync:_diff_snapshot`
- `tools/agent-sync/bin/agent-sync:_plan_sync`
...
Entrypoints:
- `tools/agent-sync/bin/agent-sync:_sync`
- `tools/agent-sync/bin/agent-sync:_apply_changes`
- `tools/agent-sync/bin/agent-sync:_commit_if_needed`
- `tools/agent-sync/bin/agent-sync:_head_commit`
- `tools/agent-sync/bin/agent-sync:_save_state`
...
- `argv`
  - Source: `main` derives it from `sys.argv[1:]` or the injected argument list in `tools/agent-sync/bin/agent-sync`.
...
- `SyncConfig`
  - Source: `_load_config` in `tools/agent-sync/bin/agent-sync`.
...
- `state`
  - Source: `_load_state(config.state_file)` in `tools/agent-sync/bin/agent-sync`.
...
- `repo_snapshot` / `source_snapshot`
  - Source: `_build_snapshot` in `tools/agent-sync/bin/agent-sync`.
...
- `source_to_repo` / `repo_to_source`
  - Source: `_diff_snapshot` or `_plan_sync` in `tools/agent-sync/bin/agent-sync`.
...
|---|---|---|---|
| `init` | CLI mode selector | `tools/agent-sync/bin/agent-sync:main` | Switches from sync execution to config generation. |
| `--force` | CLI option | `tools/agent-sync/bin/agent-sync:_parse_sync_args` / `_plan_sync` | Resolves same-file conflicts by preferring source or repo. |
| `--dry-run` | CLI option | `tools/agent-sync/bin/agent-sync:_parse_sync_args` / `_sync` | Stops the flow before file copies, state writes, and git mutations. |
| `bootstrap` | JSON config field | `tools/agent-sync/bin/agent-sync:_load_config` / `_sync` | Chooses source- or repo-owned initialization when no prior state exists. |
| `paths` | JSON config field | `tools/agent-sync/bin/agent-sync:_load_config` / `_build_snapshot` | Defines the tracked subset used to build both snapshots. |
| `branch` / `remote` | JSON config fields | `tools/agent-sync/bin/agent-sync:_load_config` / `_prepare_repo` | Controls checkout/pull/push targets and dry-run branch validation. |

### Important gates / branch controls

- `argv[0] == "init"`: selects config-generation mode instead of sync mode in `tools/agent-sync/bin/agent-sync:main`.
- `dry_run`: keeps sync planning side-effect free in `tools/agent-sync/bin/agent-sync:_sync`.
- `state is None`: switches to bootstrap handling in `tools/agent-sync/bin/agent-sync:_sync`.
- `config.bootstrap`: chooses source, repo, or error behavior for bootstrap in `tools/agent-sync/bin/agent-sync:_sync`.
- `force == "source"` / `force == "repo"`: overrides same-file conflict ownership inside `tools/agent-sync/bin/agent-sync:_plan_sync`.
...
- `tools/agent-sync/bin/agent-sync:_load_config` when validating repo/source/state paths.
- `tools/agent-sync/bin/agent-sync:_ensure_clean_repo` and `_prepare_repo` when the sync clone is unexpectedly dirty or on the wrong branch.
- `tools/agent-sync/bin/agent-sync:_plan_sync` when a path lands in `conflicts` instead of `source_to_repo` or `repo_to_source`.
- `tools/agent-sync/bin/agent-sync:_commit_if_needed` when a run unexpectedly skips or creates a commit.
- `config.state_file` contents when bootstrap behavior or repeated conflicts do not match expectations.

## Related docs

- [Project README and usage guide](../README.md)
- [Sync behavior specification](../specs/spec.md)
