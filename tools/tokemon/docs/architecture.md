...
- [`../../../apps/tokemon-menuapp/README.md`](../../../apps/tokemon-menuapp/README.md)
...
- the `tools/tokemon/bin/tokemon` Python CLI for shell-friendly CSV/JSON reports
- the `apps/tokemon-menuapp/bin/tokemon-menuapp` macOS menu-bar app, which bundles the CLI and renders range charts
...
```mermaid
flowchart LR
  User["User"] --> CLI["tools/tokemon/bin/tokemon"]
...
| --- | --- | --- |
| `tools/tokemon/bin/tokemon` | Parse CLI args, resolve ranges, orchestrate provider reads, aggregate rows, emit CSV/JSON | `tokemon [range] [--sum-by ...] [--group-by ...] [--format ...] [--provider ...]` |
| Codex adapter in `tools/tokemon/bin/tokemon` | Discover files, scan cumulative snapshots, reconcile replayed session files, optionally persist derived snapshots in SQLite | `_codex_files`, `_scan_codex_file`, `_iter_codex_usage` |
| Claude adapter in `tools/tokemon/bin/tokemon` | Discover Claude logs and dedupe per assistant message | `_claude_files`, `_iter_claude_usage` |
| Aggregation/output layer in `tools/tokemon/bin/tokemon` | Bucket normalized usage records and serialize them to CSV/JSON | `_aggregate_rows`, `_write_csv`, `_write_json` |
| `apps/tokemon-menuapp/src/TokemonMenuApp.swift` | Provide menu-bar UI, range selection, chart rendering, and refresh lifecycle | `TokemonStore`, `TokemonSnapshot`, `TokemonCommandRunner` |
| Snapshot cache | Preserve last successful app snapshots for stale-while-refresh behavior | `TokemonSnapshotCache` |
| Bundled app artifact | Freeze a copy of the CLI into the `.app` bundle | `apps/tokemon-menuapp/bin/tokemon-menuapp` |

### Primary Flows

1. A user invokes `tools/tokemon/bin/tokemon` directly or opens the menu-bar app.
...
- Bundle interface:
  - `apps/tokemon-menuapp/bin/tokemon-menuapp` copies `tools/tokemon/bin/tokemon` into the app bundle and the app executes it with `/usr/bin/python3`.
...
- Consolidate stale material in `tools/tokemon/specs/spec.md` so it matches the current indexed architecture
...
- `tools/tokemon/bin/tokemon`
- `apps/tokemon-menuapp/src/TokemonMenuApp.swift`
- `apps/tokemon-menuapp/bin/tokemon-menuapp`
- `tools/tokemon/README.md`
- `tools/tokemon/specs/spec.md`
- `apps/tokemon-menuapp/README.md`
