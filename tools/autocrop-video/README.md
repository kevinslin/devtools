...
```bash
tools/autocrop-video/bin/autocrop-video detect input.mp4
```

Print only the `ffmpeg` crop filter string:

```bash
tools/autocrop-video/bin/autocrop-video detect input.mp4 --format crop
```

Crop the file and print the detected box:

```bash
tools/autocrop-video/bin/autocrop-video crop input.mp4 output.mp4 --overwrite
```

Tune detection resolution or sample count for harder cases:

```bash
tools/autocrop-video/bin/autocrop-video detect input.mp4 --detection-width 480 --sample-count 32
```

Tune output encoding when cropping:

```bash
tools/autocrop-video/bin/autocrop-video crop input.mp4 output.mp4 --overwrite --crf 20 --preset slow
```
